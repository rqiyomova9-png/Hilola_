...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_state") == "topup_amount":
        ...
        # ══════════════════════════════════════════════════════
        # QO'LDA TO'LOV — admin tasdiqlaydi yoki bekor qiladi
        # ══════════════════════════════════════════════════════
        card = RAM.card_number or "Karta raqami o'rnatilmagan"
        context.user_data["pending_topup"] = {"user_id": uid, "amount": amount}
        await sm(context.bot, uid,
            f"💵 <b>To'lov miqdori:</b> <b>{amount:,} so'm</b>\n\n"
            f"💳 <b>Karta raqami:</b> <code>{card}</code>\n\n"
            f"<blockquote>1) Yuqoridagi kartaga aynan <b>{amount:,} so'm</b> o'tkazing.\n"
            f"2) So'ng pastdagi <b>📤 Chek yuborish</b> tugmasini bosing.\n"
            f"3) Chek rasmini yuboring — admin tekshirib tasdiqlaydi yoki bekor qiladi.</blockquote>",
...
class _WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        """Avtomatik to'lov o'chirilgan — webhook qabul qilinmaydi."""
        self.send_response(200)
        self.send_header("Content-Type", "application/json")
        self.end_headers()
        self.wfile.write(b'{"ok": true, "disabled": true}')
...
def main():
    ...
    # ── Railway health check server (faqat GET / uchun) ──
    ...
    async def _startup_notify(context_job):
        try:
            ...
            if ok:
                ...
                sql_msg = "🟢 SQL ishlayapti"
            else:
                DB_STATUS.update({"storage_ok": False, "ram_only": True, "last_err": now_str})
                sql_msg = "🔴 SQL ishlamayapti"
            await context_job.bot.send_message(
                ADMIN_ID,
                f"<b>Azizbek Dasturchi Ishga tushirib berdi!</b>\n"
                f"Botni Bemalol Foydalanishingiz mumkin Hilolaxon\n\n"
                f"{sql_msg}",
                parse_mode="HTML")
        except Exception as e:
